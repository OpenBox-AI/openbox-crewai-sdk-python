...
stages, sending governance evaluations to OpenBox Core via the governance module.
...
_govern_query_depth: ContextVar[int] = ContextVar("openbox_db_govern_depth", default=0)
_psycopg2_patched = False

_span_processor: Any = None


def configure(span_processor: Any) -> None:
...
_WITH_PREFIX_RE = re.compile(r"^\s*WITH\b.*?\)\s*(?=\w)", re.DOTALL | re.IGNORECASE)
_VERB_RE = re.compile(
    r"^\s*(SELECT|INSERT|UPDATE|DELETE|MERGE|CREATE|DROP|ALTER|TRUNCATE|"
    r"CALL|DO|COPY|BEGIN|COMMIT|ROLLBACK|EXPLAIN)\b",
    re.IGNORECASE,
)
...
def _build_db_span_data(
    ...
    from . import governance as _hook_gov
...
def _run_governed_query_sync(
    ...
    depth = _govern_query_depth.get()
    if depth > 0:
        return query_method(*args, **kwargs)
    token = _govern_query_depth.set(depth + 1)
    ...
    try:
        ...
        _govern_query_depth.reset(token)
...
async def _run_governed_query_async(
    ...
    Opens a short-lived OTel span named ``{operation} {db_system}`` so that
    ``get_current_span()`` inside ``_build_db_span_data`` returns a DB-shaped
    span rather than any leaked outer span (e.g. an agent-wrapping span).
    The wrapt-driver paths don't sit inside OTel's own DB instrumentor span
    at this point, so without this the payload would inherit the wrong name.
    """
    from openbox.core.errors import GovernanceBlockedError
    ...
    tracer = otel_trace.get_tracer("openbox.db")
    ident = _db_identifier(db_system, host, port, db_name)
    try:
        with tracer.start_as_current_span(
            f"{operation} {db_system}", kind=otel_trace.SpanKind.CLIENT
        ) as current_span:
            started_sd = _build_db_span_data(
                current_span, db_system, db_name, operation, stmt, host, port,
                "started",
...
def _extract_psycopg2_context(cursor_instance: Any) -> tuple[str, int, str | None]:
    try:
        dsn_params = cursor_instance.connection.get_dsn_parameters() or {}
        host = dsn_params.get("host", "unknown")
        port = int(dsn_params.get("port", 5432))
        db_name = dsn_params.get("dbname")
        return host, port, db_name
    except (AttributeError, TypeError, ValueError):
        return "unknown", 5432, None


def install_psycopg2_hooks() -> bool:
    """Install direct governance hooks on psycopg2 cursor methods."""
    global _psycopg2_patched
    if _psycopg2_patched:
        return True
    ...
    def _psycopg2_governance_wrapper(wrapped, instance, args, kwargs):
        query = args[0] if args else ""
        operation = _classify_sql(query)
        stmt = str(query)[:2000]
        host, port, db_name = _extract_psycopg2_context(instance)
    ...
    for module, method in methods:
        try:
            wrapt.wrap_function_wrapper(module, method, _psycopg2_governance_wrapper)
            _installed_patches.append((module, method))
            patched += 1
        except (AttributeError, TypeError, ImportError) as e:
            logger.debug(f"psycopg2 governance hook failed for {method}: {e}")

    if patched > 0:
        _psycopg2_patched = True
        logger.info(f"psycopg2 governance hooks installed: {patched}/{len(methods)} methods")
        return True

    logger.debug("No psycopg2 methods patched for governance")
    return False


# ═══════════════════════════════════════════════════════════════════════════════
# aiopg — wrapt wrapper (async DBAPI for Postgres; no CursorTracer async in 0.55b)
# ═══════════════════════════════════════════════════════════════════════════════

_aiopg_patched = False


def _extract_aiopg_context(cursor_instance: Any) -> tuple[str, int, str | None]:
    try:
        conn = cursor_instance.connection._conn  # aiopg.Connection → underlying psycopg2
        dsn_params = getattr(conn, "get_dsn_parameters", lambda: {})() or {}
        host = dsn_params.get("host", "unknown")
        port = int(dsn_params.get("port", 5432))
        db_name = dsn_params.get("dbname")
        return host, port, db_name
...
def install_aiopg_hooks() -> bool:
    """Install governance hooks on aiopg cursor.execute via wrapt wrapping."""
    global _aiopg_patched
    if _aiopg_patched:
        return True
    ...
    async def _aiopg_governance_wrapper(wrapped, instance, args, kwargs):
        query = args[0] if args else ""
        operation = _classify_sql(query)
        stmt = str(query)[:2000]
        host, port, db_name = _extract_aiopg_context(instance)

        return await _run_governed_query_async(
            wrapped, args, kwargs,
            db_system="postgresql", db_name=db_name, operation=operation,
            stmt=stmt, host=host, port=port, cursor=instance,
        )

    methods = [
        ("aiopg.connection", "Cursor.execute"),
        ("aiopg.connection", "Cursor.executemany"),
    ]
    patched = 0
    for module, method in methods:
        try:
            wrapt.wrap_function_wrapper(module, method, _aiopg_governance_wrapper)
            _installed_patches.append((module, method))
            patched += 1
        except (AttributeError, TypeError, ImportError) as e:
            logger.debug(f"aiopg governance hook failed for {method}: {e}")

    if patched > 0:
        _aiopg_patched = True
        logger.info(f"aiopg governance hooks installed: {patched}/{len(methods)} methods")
        return True

    logger.debug("No aiopg methods patched for governance")
    return False


# ═══════════════════════════════════════════════════════════════════════════════
# psycopg 3 async — wrapt wrapper (no OTel async cursor path on 0.55b)
# ═══════════════════════════════════════════════════════════════════════════════

_psycopg3_async_patched = False


def _extract_psycopg3_context(cursor_instance: Any) -> tuple[str, int, str | None]:
    try:
        info = cursor_instance.connection.info
        return (
            info.host or "unknown",
            int(info.port or 5432),
            info.dbname,
        )
    except (AttributeError, TypeError, ValueError):
        return "unknown", 5432, None
...
def install_psycopg3_async_hooks() -> bool:
    """Install governance hooks on psycopg.AsyncCursor via wrapt wrapping."""
    global _psycopg3_async_patched
    if _psycopg3_async_patched:
        return True
    ...
    async def _psycopg3_governance_wrapper(wrapped, instance, args, kwargs):
        query = args[0] if args else ""
        operation = _classify_sql(query)
        stmt = str(query)[:2000]
        host, port, db_name = _extract_psycopg3_context(instance)
    ...
    methods = [
        ("psycopg", "AsyncCursor.execute"),
        ("psycopg", "AsyncCursor.executemany"),
    ]
    patched = 0
    for module, method in methods:
        try:
            wrapt.wrap_function_wrapper(module, method, _psycopg3_governance_wrapper)
            _installed_patches.append((module, method))
            patched += 1
        except (AttributeError, TypeError, ImportError) as e:
            logger.debug(f"psycopg 3 async governance hook failed for {method}: {e}")

    if patched > 0:
        _psycopg3_async_patched = True
        logger.info(
            f"psycopg 3 async governance hooks installed: {patched}/{len(methods)} methods"
        )
        return True

    logger.debug("No psycopg 3 async methods patched for governance")
    return False
...
def uninstrument_all() -> None:
    ...
    global _asyncpg_patched, _psycopg2_patched, _aiopg_patched, _psycopg3_async_patched
    _uninstall_cursor_tracer_hooks()

    # Unwrap every wrapt-wrapped function recorded across all DB drivers.
    # _installed_patches entries look like ("asyncpg.connection", "Connection.execute")
    # — class.method on a module; split into (module.class, method) for OTel's unwrap.
    try:
        from opentelemetry.instrumentation.utils import unwrap

        for module_path, method in list(_installed_patches):
            try:
                if "." in method:
                    cls_name, attr = method.rsplit(".", 1)
                    unwrap(f"{module_path}.{cls_name}", attr)
                else:
                    unwrap(module_path, method)
            except Exception:
                logger.debug(
                    "Failed to unwrap %s.%s", module_path, method, exc_info=True
                )
    except ImportError:
        logger.debug("OTel unwrap helper not available")
